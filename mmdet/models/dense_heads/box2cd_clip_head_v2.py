...
class Box2cd_CLIP_Head_v2(BaseModule):
    def __init__(self,
                 num_classes,
                 in_channels,
                 ...
                 loss_not_inst_weight=[0.0,1],
                 conv_cfg=None,
                 norm_cfg=None,
                 use_dcn_in_tower=False,
                 type_dcn=None,
                 init_cfg=None):
        super(Box2cd_CLIP_Head_v2, self).__init__(init_cfg=init_cfg)
        ...
        self.loss_sscr = build_loss(loss_sscr)
        self.loss_similarity=build_loss(loss_similarity)
        self.loss_not_ins_weight=loss_not_inst_weight
    ...
    def init_weights(self):
        super(Box2cd_CLIP_Head_v2, self).init_weights()
    ...
    def forward(self, feats, eval=False):
        ...
        # for i in ins_pred:
        #     print('ins_pred ',i.size())
        # torch.Size([2, 1600, 128, 128])
        # torch.Size([2, 1296, 128, 128])
        # torch.Size([2, 576, 64, 64])
    ...
    def loss(self,
             ...
             cfg,
             feats2,
             x1, x2, score_map1,score_map2,mask_and_score_list,
             gt_bboxes_ignore=None):

        device = gt_label_list[0].device

        loss_s1,loss_s2,loss_s3=self.loss_similarity(x1,x2,score_map1,score_map2,mask_and_score_list)
        ...
        # not_ins_ind_label_list=[]
        # for b in range(num_imgs):
        #     batch=[]
        #     for label in ins_ind_label_list[b]:
        #         not_ins_ind_label = torch.ones(label.size()[0], dtype=torch.bool, device=device)
        #         not_ins_ind_label=not_ins_ind_label^label
        #         batch.append(not_ins_ind_label)
        #     not_ins_ind_label_list.append(batch)


        # print(len(ins_ind_label_list),[i.size() for i in ins_ind_label_list[0]])
        # # print(ins_ind_label_list[0][2])
        ...
        # not_ins_preds = [torch.cat([ins_preds_level_img[ins_ind_labels_level_img, ...]
        #                             for ins_preds_level_img, ins_ind_labels_level_img in
        #                             zip(ins_preds_level, ins_ind_labels_level)], 0)
        #                  for ins_preds_level, ins_ind_labels_level in zip(ins_preds, zip(*not_ins_ind_label_list))]
        ...
        #     S=ins_preds[i].size()[0]*self.loss_not_ins_weight[1] #采样个数s=正样本个数*倍率
        #     N=not_ins_preds[i].size()[0]
        #     index = torch.LongTensor(random.sample(range(N), S)).to(device)
        #     not_ins_preds[i] = torch.index_select(not_ins_preds[i], 0, index)
        #
        # #计算负样本loss
        # loss_not_ins = 0
        # for not_ins_pred  in not_ins_preds:
        #     if not_ins_pred.size()[0] == 0:
        #         continue
        #     mask_pred = torch.sigmoid(not_ins_pred.unsqueeze(dim=1))
        #     loss_not_ins+=self.loss_not_ins_weight[0]*mask_pred.mean()
        ...
        for ins_pred, box_mask, img_target, lst_target in zip(ins_preds, ins_labels, img_target, lst_target):
            ...
            # print('mask_scores_phi ',mask_scores_phi.size())
            # print(mask_scores_phi)
        ...
        if len(loss_project)==0:
            loss_project = loss_s1*0
            loss_not_ins = loss_s1*0
            loss_levelset = loss_s1 * 0
        else:
        ...
        return dict(
            ...
            loss_cate=loss_cate,
            loss_sscr=loss_sscr,
            loss_similarity1=loss_s1,
            loss_sim2=loss_s2,
            loss_sim3=loss_s3,
            # loss_not_ins=loss_not_ins
        )
