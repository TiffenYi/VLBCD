...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        #torch.Size([2, 1024])  torch.Size([2, 1024, 16, 16])
...
class CLIPResNetWithAttention(nn.Module):
    ...
    def __init__(self, layers, output_dim=1024, input_resolution=224, width=64, pretrained=None, **kwargs):
        ...
        embed_dim = width * 32  # the ResNet feature dimension  64*32=2048
...
class CLIPTextContextEncoder(nn.Module):
    ...
    def forward2(self, text, context):
        ...
        eos_indx = text.argmax(dim=-1) + N2
        # print(eos_indx,eos_indx.shape)
        eos_indx = eos_indx.reshape(-1, K).unsqueeze(1).reshape(-1)
