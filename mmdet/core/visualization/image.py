...
__all__ = [
    ...
    'imshow_det_bboxes', 'imshow_gt_det_bboxes','imshow_det_cdmask','draw_cd_masks'
]
...
def draw_masks(ax, img, masks, color=None, with_edge=True, alpha=0.5):
    ...
    p = PatchCollection(
        polygons, facecolor='none', edgecolors='w', linewidths=0.5, alpha=0.8)
    ax.add_collection(p)

    return ax, img


def draw_cd_masks(ax, img, masks, color=None, with_edge=True, alpha=0.5):
    """Draw masks on the image and their edges on the axes.
...
def imshow_det_cdmask(img,
    ...
    if bboxes is not None:
        ...
        center_x = ((bboxes[:, 0:1] + bboxes[:, 2:3]) / 2.0).astype(np.int32)
        center_y = ((bboxes[:, 1:2] + bboxes[:, 3:4]) / 2.0).astype(np.int32)
        positions = np.concatenate((center_x, center_y), axis=1)
    ...
    if segms is not None:
        ...
        draw_cd_masks(ax, img, segms, colors, with_edge=True)
