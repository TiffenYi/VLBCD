...
class Box2Cd_CLIP_v4(SingleStageBox2CdDetector):
    ...
    def __init__(self,
        ...
        super(Box2Cd_CLIP_v4,self).__init__(backbone, neck, bbox_head, train_cfg,
    ...
    # def extract_feat(self, img1, img2):
    #     """Extract features from images. 相减+拼接"""
    #     x1 = self.backbone(img1)
    #     x2 = self.backbone(img2)
    #     x = []
    #     for i in range(4):
    ...
    #         f = torch.cat([x1[i] , x2[i]], dim=1)
    #         f = conv_1_1(f)
    #
    #         # 相减融合
    #         sub = torch.abs(x1[i]-x2[i])
    ...
    #         x.append(f)
    #
    #     temp = [torch.abs(x1[4][0] - x2[4][0]), torch.abs(x1[4][1] - x2[4][1])]
    #     x.append(temp)
    #     return x
    ...
    def visulize_all_channel_into_one(self, feature_map, i):
        ...
        # 热力图保存位置
        plt.savefig('heatmap-whu-vit/heat{}.png'.format(i + 1), dpi=3 * height)
        plt.savefig('heatmap-whu-vit/heat{}.png'.format(i + 1), dpi=256, bbox_inches='tight', pad_inches=0)
    ...
    def simple_test(self, img1, img2, img_meta, rescale=False):
        ...
        for i in range(channel_num):
            out_file = os.path.join(out_dir, os.path.splitext(img_meta[0]['ori_filename'])[0] + '_' + str(i) +
                                    os.path.splitext(img_meta[0]['ori_filename'])[1])
