...
class Box2CdCondInst(SingleStageBox2CdDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(Box2CdCondInst, self).__init__(backbone, neck, bbox_head, train_cfg,
    ...
    def forward_train(self,
        ...
        if gt_masks is not None:
            ...
            for masks in gt_masks:
                ...
                tensor_masks.append(
                    masks.to_tensor(dtype=torch.uint8, device=img1.device))
        ...
        loss_mask = self.mask_head.loss(img1-img2, img_metas, mask_pred, gt_inds, gt_bboxes,
                                        gt_masks, gt_labels)
        losses.update(loss_mask)
        return losses

    def simple_test(self, img1,img2, img_metas, rescale=False):
        feat = self.extract_feat(img1,img2)
        outputs = self.bbox_head.simple_test(
            feat, self.mask_head.param_conv, img_metas, rescale=rescale,test_cfg=self.test_cfg)
