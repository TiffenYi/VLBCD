...
class BoxSOLOv2Head_v4(BaseModule):
    def __init__(self,
                 num_classes,
                 in_channels,
                 ...
                 type_dcn=None,
                 init_cfg=None):
        super(BoxSOLOv2Head_v4, self).__init__(init_cfg=init_cfg)
    ...
    def init_weights(self):
        super(BoxSOLOv2Head_v4, self).init_weights()
    ...
    def loss(self,
        ...
        #
        #     box_mask=self.resize_box_mask(bbox,256,256,64,64).to(levelset_f.device)
        #     loss=self.loss_boxlevelset(levelset_f,box_mask).unsqueeze(dim=0)
        ...
        #     f[idx]=self.sscr_cam(f[idx])
        # for idx in range(len(_f)):
        #     _f[idx]=self.sscr_cam(_f[idx])
