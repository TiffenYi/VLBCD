...
class BoxSOLOv2Head_v3(BaseModule):
    def __init__(self,
                 num_classes,
                 in_channels,
                 ...
                 loss_boxpro=None,
                 loss_levelset=None,
                 loss_boxlevelset=None,
                 conv_cfg=None,
                 norm_cfg=None,
                 use_dcn_in_tower=False,
                 type_dcn=None,
                 init_cfg=None):
        super(BoxSOLOv2Head_v3, self).__init__(init_cfg=init_cfg)
        ...
        self.loss_boxlevelset=build_loss(loss_boxlevelset)
    ...
    def _init_layers(self):
        ...
        self.box_mask_conv=nn.Sequential(
            nn.Conv2d(5, 1, kernel_size=1, stride=1),
        ...
        # params = torch.rand(4,1,64,64,requires_grad=True)
        # self.params = nn.Parameter(params)

    def init_weights(self):
        super(BoxSOLOv2Head_v3, self).init_weights()
        ...
        normal_init(self.box_mask_conv, std=0.01)
    ...
    def forward(self, feats, eval=False):
        ...
        # 7-25决定新增的模块  学习一个mask
        box_mask=self.box_mask_conv(levelset_feats)
        # box_mask = self.params
        levelset_feats = levelset_feats*box_mask
    ...
    def resize_box_mask(self,box_list, iw=256, ih=256, nw=64, nh=64):
        idx = 0
        mask = torch.ones(size=(nw, nh))
        for x1, y1, x2, y2 in box_list:
            box_list[idx][0] = int(int(x1) * (nw / iw))
            box_list[idx][1] = int(int(y1) * (nh / ih))
            box_list[idx][2] = int(int(x2) * (nw / iw))
            box_list[idx][3] = int(int(y2) * (nh / ih))
            # print(x1, y1, x2, y2)
    ...
    def loss(self,
        ...
        # mask box levelset loss
        # loss_box_levelset=[]
        # boxMask_list=[]
        # box_size=[64,64,64,64]
        # # box_size = [64 for i in range(len(gt_bbox_list))]
        # for idx in range(len(gt_bbox_list)):
        #     bbox=gt_bbox_list[idx]
        #     levelset_f=levelset_feats[idx]
        #     bbox = [b.tolist() for b in bbox]
        #     bbox = torch.tensor(bbox,dtype=torch.int32)
        #
        #     bbox_mask=self.resize_box_mask(bbox,256,256,box_size[idx],box_size[idx]).to(levelset_f.device)
        #     boxMask_list.append(bbox_mask)
        #     loss=self.loss_boxlevelset(levelset_f,bbox_mask).unsqueeze(dim=0)
        #     loss_box_levelset.append(loss)
        # loss_boxlevelset=torch.cat(loss_box_levelset).mean()
        ...
        # fxs,fys=feats2
        # # box_size = [64 for i in range(len(gt_bbox_list))]
        # assert len(fxs) == len(fys)
        # for i in range(2,len(fxs)):
        #     fx=fxs[i]
        #     fy=fys[i]
        #     for idx in range(len(gt_bbox_list)):
        #         bbox=gt_bbox_list[idx]
        ...
        #         bbox = torch.tensor(bbox,dtype=torch.int32)
        #
        #         bbox_mask=self.resize_box_mask(bbox,256,256,fx.size()[-2],fx.size()[-1]).to(fx.device)
        #         loss=self.loss_boxlevelset(feat,bbox_mask).unsqueeze(dim=0)
        #         loss_box_levelset.append(loss)
        # loss_boxlevelset=torch.cat(loss_box_levelset).mean()
        ...
        # print(len(ins_ind_label_list),len(ins_ind_label_list[0]))
        # ins
        ...
        for ins_pred, box_mask, img_target, lst_target in zip(ins_preds, ins_labels, img_target, lst_target):
            ...
            # print('mask t',box_mask_target.size())  #   ？, 1, 64, 64     64-64-32-16
        ...
        #     loss_levelset=loss_levelset,
        #     loss_cate=loss_cate,
        #     loss_boxlevelset=loss_boxlevelset)
